...
import { listMySuperAllocations } from '@/api/business'
...
const facilityName = computed(() => auth.user?.facilityName || '本超市')

const loadData = async () => {
  loading.value = true
  try {
    const res = await listMySuperAllocations()
    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <div>
            <span class="title">收物表</span>
            <span class="subtitle">超市名称：{{ facilityName }}</span>
          </div>
        </div>
      </template>
      ...
      >
        ...
        <el-empty v-if="!allocations.length && !loading" slot="empty" description="暂无收物记录" />
      </el-table>
    </el-card>
  </div>
</template>
